...
- 芸人・コンビ・グループ: 290 組（初版100 + 追加190）
- 落語家: 52 名（初版18 + 追加34）／エンティティ計 342
- 番組・配信コンテンツ: 34 件
- 公式/正当な動画・チャンネルリンク: 約160 件
- 質問数: 17（うち任意5）／診断タイプ数: 12

追加分は並列リサーチで実在・所属・現役/物故・公式URLを検証したもののみを収録。
検証が間に合わなかったバッチ（新世代/女性・THE W/非吉本/江戸落語/近年M-1 等）は
架空データで埋めず未収録とし、後日追加予定。`src/data/catalog.ts` のビルド時ガードで
ID重複が無いことを保証（テストでも検証）。
